...
// org.littletonrobotics.frc2023.Constants;
  ...
  */
  ...
  public void periodic() {
    ...
    for (int instanceIndex = 0; instanceIndex < io.length; instanceIndex++) {
      ...
      // Loop over frames in each instance 
      for (int frameIndex = 0; frameIndex < inputs[instanceIndex].timestamps.length; frameIndex++) {
          ...
          case 1:
            ...
            robotPose3d =
                cameraPose.transformBy(
                    GeoUtil.pose3dToTransform3d(cameraPoses[instanceIndex]).inverse());
            break;
        ...
        if (robotPose3d.getX() < -fieldBorderMargin
            || robotPose3d.getX() > deathstarVision.fieldLength + fieldBorderMargin
            || robotPose3d.getY() < -fieldBorderMargin
            || robotPose3d.getY() > deathstarVision.fieldWidth + fieldBorderMargin
        ...
        for (int i = (values[0] == 1 ? 9 : 17); i < values.length; i++) {
          ...
          Optional<Pose3d> tagPose = deathstarVision.aprilTags.getTagPose((int) values[i]);
      ...
      if (inputs[instanceIndex].demoFrame.length > 0) {
        ...
        Pose3d fieldToCameraPose =
            new Pose3d(poseSupplier.get())
                .transformBy(GeoUtil.pose3dToTransform3d(cameraPoses[instanceIndex]));
    ...
    for (Map.Entry<Integer, Double> detectionEntry : lastTagDetectionTimes.entrySet()) {
      if (Timer.getFPGATimestamp() - detectionEntry.getValue() < targetLogTimeSecs) {
        allTagPoses.add(deathstarVision.aprilTags.getTagPose(detectionEntry.getKey()).get());
      }
    }
    Logger.getInstance()
        .recordOutput(
